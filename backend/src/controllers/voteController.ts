...
import { generateZKProof, generateVoteHash, verifyZKProof } from '../utils/zkProof';

export const castVote = async (req: AuthRequest, res: Response) => {
    try {
        const { election_id, option_id } = req.body as unknown as { election_id: string; option_id: string };
        ...
        const zkProof = generateZKProof(userId!, option_id, election_id);
        const voteHash = generateVoteHash(option_id, election_id, timestamp);
...
export const verifyVote = async (req: AuthRequest, res: Response) => {
    try {
        const { voteHash } = req.params as unknown as { voteHash: string };
      ...
      `)
        ...
        const isValid = verifyZKProof(vote.zk_proof);
...
export const getUserVoteStatus = async (req: AuthRequest, res: Response) => {
    try {
        const { election_id } = req.params as unknown as { election_id: string };
        const userId = req.user?.id;
...
export const getAllVotes = async (req: AuthRequest, res: Response) => {
    try {
        const { election_id } = req.params as unknown as { election_id: string };
        ...
        res.json({
            ...
            votes: votes.map(vote => ({
                ...
                zkProof: vote.zk_proof,
                timestamp: vote.timestamp,
                verified: verifyZKProof(vote.zk_proof)
            }))
        });
    } catch (error) {
